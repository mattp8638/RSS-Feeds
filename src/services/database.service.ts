...
export class DatabaseService {
  ...
  const defaultFeeds: Omit<Feed, 'created_at' | 'updated_at'>[] = [
  ...
  if (knownUnavailableFeedIds.length > 0) {
    await this.pool.query(
      'UPDATE feeds SET enabled = FALSE, updated_at = NOW() WHERE id = ANY($1::text[])',
      [knownUnavailableFeedIds]
    );
  }
}

  public async upsertArticles(articles: Omit<Article, 'created_at'>[]): Promise<void> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  public async fetchLatestArticles(limit: number = 1000): Promise<Article[]> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  public async fetchArticlesForSource(sourceId: string): Promise<Article[]> {
    if (!this.pool) throw new Error('Database not initialized');
    const { rows } = await this.pool.query(
      `
      SELECT id, title, summary, link, published, source, category, source_id, created_at
  ...
  public async fetchArticlesLastNDays(days: number): Promise<Article[]> {
    if (!this.pool) throw new Error('Database not initialized');
    const { rows } = await this.pool.query(
      `
      SELECT id, title, summary, link, published, source, category, source_id, created_at
  ...
  public async deleteOlderThan(days: number): Promise<void> {
    if (!this.pool) throw new Error('Database not initialized');
    ...
    logger.info(`Deleted ${result.rowCount ?? 0} old articles (older than ${days} days)`);
  }
  ...
  public async getEnabledFeeds(): Promise<Feed[]> {
    if (!this.pool) throw new Error('Database not initialized');
    const { rows } = await this.pool.query('SELECT * FROM feeds WHERE enabled = TRUE ORDER BY name');
    return rows as Feed[];
  }

  public async getFeedById(id: string): Promise<Feed | undefined> {
    if (!this.pool) throw new Error('Database not initialized');
    const { rows } = await this.pool.query('SELECT * FROM feeds WHERE id = $1', [id]);
    return rows[0] as Feed | undefined;
  }

  public async createFeed(feed: Omit<Feed, 'created_at' | 'updated_at'>): Promise<Feed> {
    if (!this.pool) throw new Error('Database not initialized');
  ...
  public async updateFeed(id: string, updates: Partial<Omit<Feed, 'id' | 'created_at' | 'updated_at'>>): Promise<Feed> {
    if (!this.pool) throw new Error('Database not initialized');

    const existing = await this.getFeedById(id);
    if (!existing) throw new Error(`Feed with id '${id}' not found`);
    ...
    if (!updated) throw new Error('Failed to update feed');
    return updated;
  }

  public async deleteFeed(id: string): Promise<void> {
    if (!this.pool) throw new Error('Database not initialized');
